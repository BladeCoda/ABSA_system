...
class Corpus:
    ...
    #清除Instance中的aspect term,主要用于修改测试集合
    def clean_tags(self):
...
class BaselineAspectExtractor():
    ...
    def tag(self, test_instances):
        ...
        for i in test_instances:
            ...
            clones.append(i_)
        #对每个句子转化为（term,offset）的list
        return clones

#AspectCategory检测的baseline方法
#采用KNN分类的方法
...
def main(argv=None):
    ...
    try:
        ...
        print("Unexpected error:", sys.exc_info()[0])#不规范的文件
        raise
    ...
    #如果没有指定test文件，那么就要进行train和test的切分
    else:
